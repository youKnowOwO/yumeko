    ...
    async exec(msg, { track, isSearch, dontBind }) {
        ...
        if (typeof track === "string") {
            ...
            if (response.loadType === "PLAYLIST_LOADED") {
                ...
                music.add(msg.author, trck);
                if (music.song)
                    msg.ctx.send(msg.ctx.lang("COMMAND_MUSIC_PLAY_ADD_SONG", trck.info.title));
            }
        }
        else
...
default_1 = __decorate([
    decorators_1.DeclareCommand("play", {
        aliases: ["play", "p"],
        description: {
            content: (msg) => msg.ctx.lang("COMMAND_MUSIC_PLAY_DESCRIPTION"),
        ...
        args: [
            ...
            },
            {
                identifier: "track",
                match: "rest",
                prompt: (msg) => msg.ctx.lang("COMMAND_MUSIC_PLAY_PROMPT"),
                type: (msg, content) => {
                    try {
