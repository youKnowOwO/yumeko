...
@DeclareCommand("skip", {
    aliases: ["skip"],
    description: {
        content: (msg): string => msg.ctx.lang("COMMAND_MUSIC_SKIP_DESCRIPTION"),
        usage: "skip",
        examples: ["skip", "--force"]
    },
    category: "music",
    ...
    public async exec(msg: Message, { forced }: { forced: boolean }): Promise<Message> {
        ...
        if (!forced) {
            ...
            if (listeners > 3 && music.song!.requester.id !== msg.author.id) {
                if (music.skipVotes.includes(msg.author)) return msg.ctx.send(msg.ctx.lang("COMMAND_MUSIC_SKIP_ALREADY_VOTE"));
