...
export default class TypeMemberhuman implements Type {
    ...
    public exec(msg: Message, content: string): GuildMember {
        ...
        if (member.user.bot) throw new CustomError("!PARSING", msg.guild!.loc.get("TYPE_HUMAN_BOT"));
        if (member.user.id === msg.author.id) throw new CustomError("!PARSING", msg.guild!.loc.get("TYPE_HUMAN_SELF"));
        return member;
    }
}